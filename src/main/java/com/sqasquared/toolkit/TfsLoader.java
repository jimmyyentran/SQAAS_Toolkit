...
public class TfsLoader extends Loader<DataObject> {
    ...
    public void loadWorkingTree(String apiUrl, String parentWit, String
            parentId, String childWit) throws IOException {
        JsonObject response = TfsWrapper.getWorkingTree(apiUrl, parentWit,
                parentId, childWit);
        // Convert JsonArray to HashMap of TfsObjects
        Gson gson = new GsonBuilder().create();
        Type tfsObjectType = new TypeToken<HashMap<String, TfsObject>>() {
        }.getType();
        HashMap<String, DataObject> tfsObjectMap = gson.fromJson(response,
                tfsObjectType);
